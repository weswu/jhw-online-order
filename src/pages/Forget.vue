    ...
    <div class="container">
      <div class="login">
        <form class="login-bd">
          ...
          <mu-text-field label="新密码" hintText="6-16位, 数字、字母、符号至少包含两种, 同时不能包含中文和空格" type="password" v-model="password" @change="passwordInput" :errorText="passwordError" fullWidth />
          <div class="login-ft">
            <p>
              <mu-raised-button @click="submit" label="立即修改" type="submit" primary />
            </p>
            <mu-flat-button @click="toLogin" label="返回登录" />
          </div>
        </form>
      </div>
    </div>
